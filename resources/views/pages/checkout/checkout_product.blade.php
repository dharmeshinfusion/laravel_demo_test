                                ...
                                <div class="product_rate_info">
                                    <div class="quantity buttons_added" style="float: left">
                                        <input type="button" value="-" class="minus"
                                            style="margin: 0px !important; padding: 0px;height:30px">
                                        <input type="number" step="1" min="1" max="" name="quantity"
                                            value="{{ $list['quantity'] }}" title="Qty"
                                            class="input-text qty text quantity" size="4" pattern=""
                                            inputmode="" style="margin: 0px !important; padding: 0px;height:30px">
                                        <input type="button" value="+" class="plus"
                                            style="margin: 0px !important; padding: 0px;height:30px">
                                    </div>
                                    <span class="pqt" style="float: left">{{ $list['total_price'] }}</span>
                                </div>
                ...
                <div class="details_card">
                    <div class="proced_payment">
                        <a href="{{ route('home') }}" class="btn btn-danger w-100"
                            style="background-color:#dc3545;color:#FFF">No Item In Cart...</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        $(document).ready(function() {
            $('.plus').click(function() {
                ...
                swal({
                    ...
                    showCancelButton: true,
                    confirmButtonColor: "#e69a2a",
                    confirmButtonText: "Yes, Plus Cart it!",
                    cancelButtonText: "No, cancel please!",
                    closeOnConfirm: false,
                    closeOnCancel: false
                }, function(isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            url: "{{ route('cartitem', ['']) }}" + "/" + id,
                            type: 'post',
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "quantity": quantity,
                                "flage": 'Cart Item Successfully Added 🔥 !'
                            },
                            success: function(msg) {
                                if (msg.status == 'success') {
                                    swal({
                                            title: "Add",
                                            text: "Cart Item Successfully Add",
                                            type: "success"
                                        },
                                        function() {
                                            location.reload();
            ...
            $('.minus').click(function() {
                ...
                swal({
                    ...
                    showCancelButton: true,
                    confirmButtonColor: "#e69a2a",
                    confirmButtonText: "Yes, Minus Cart it!",
                    cancelButtonText: "No, cancel please!",
                    closeOnConfirm: false,
                    closeOnCancel: false
                }, function(isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            ...
                            data: {
                                "_token": "{{ csrf_token() }}",
                                "quantity": quantity,
                                "flage": 'Cart Item Successfully Remove 🔥 !'
                            },
                            success: function(msg) {
                                if (msg.status == 'success') {
                                    swal({
